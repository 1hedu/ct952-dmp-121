...
- **`OS_GetSysTimer` (`0x59838`) tick is NOT frozen.** It reads via pointer `[0x400398f8]=0x4002e320`; the tick at `0x4002e354` advances **~4900/s** live. (`OSDSS_Monitor 0x591b4` arms when `OS_GetSysTimer()-__dwOSDSSCheckTime(0x396e) > 0xe260`; the tick would cross that in ~12 s if the monitor re-ran.)
