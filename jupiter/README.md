...
| `emulator/` licheeEmu (device emulation) | `emu/` (ct952emu) | Full-machine emulator: a from-scratch, verified SPARC V8 CPU core (bit-exact vs native GCC on a stress payload) + the CT909 memory map + core device models (timers, UARTs, interrupt controller, PROC2 mailbox). Boots the stock `DVD909.rom` and runs millions of firmware instructions with zero illegal-instruction traps; its I/O-access inventory root-causes the boot stall (the packed-ROM section table + custom decompressor staged by the earlier `dsu_boot`). See [`emu/BRINGUP.md`](emu/BRINGUP.md). |
...
- `jusbhid.c` (USB HID boot-keyboard decoder) is **complete and verified**
  (`test/test_usbhid.c`, run native + big-endian SPARC): an 8-byte boot
  report becomes press/release events with correct chord ordering
