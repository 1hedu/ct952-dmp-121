...
(cd build/native && ./test_render && ./test_render2 && ./test_fb \
    && ./test_gpu && ./test_spr && ./test_audio) | tee native.log || FAIL=1
...
(cd build/sparc && qemu-sparc64 ./test_render && qemu-sparc64 ./test_render2 \
    ...
    && qemu-sparc64 ./test_spr && qemu-sparc64 ./test_audio) \
...
mkdir -p build/fw
for f in jnes jgb jsnes jgen jaudio jrgb2yuv jfb jcodec_ct952 jgpu jgpu_ct952 jspr jshim_ct952 japp; do
